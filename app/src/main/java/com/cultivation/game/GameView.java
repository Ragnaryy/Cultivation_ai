...
public class GameView extends View {
    ...
    private void drawPlayer(Canvas canvas) {
        ...
        int screenX = (int)(player.position.x * scaleX);
        int screenY = (int)(player.position.y * scaleY);
    ...
    private void drawMonsters(Canvas canvas) {
        if (gameWorld == null || gameWorld.getMonsters() == null) return;

        for (Monster monster : gameWorld.getMonsters()) {
            if (monster.isAlive()) {
                int screenX = (int)(monster.position.x * scaleX);
                int screenY = (int)(monster.position.y * scaleY);
    ...
    private void drawResources(Canvas canvas) {
        if (gameWorld == null || gameWorld.getResources() == null) return;

        for (Resource resource : gameWorld.getResources()) {
            int screenX = (int)(resource.position.x * scaleX);
            int screenY = (int)(resource.position.y * scaleY);
    ...
    private void drawUI(Canvas canvas) {
        ...
        if (gameWorld != null) {
            ...
            Player player = gameWorld.getPlayer();
            if (player != null) {
                canvas.drawText("Позиция: " + player.position.x + "," + player.position.y, 10, 80, paint);
            }
        }
    }
